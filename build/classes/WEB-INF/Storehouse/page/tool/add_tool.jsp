...
<title>ลงทะเบียนวัสดุ/อุปกรณ์</title>
</head>
<body>
...
<br><br>
	<div class="container">
		<div class="col-md-3">
				...
				<p style="font-size: 28px; font-weight: 700; color: white;">การลงทะเบียนวัสดุ/อุปกรณ์</p>
			</div>
		</div>
		<br><br><br>
		...
		<div class="col-md-offset-3 col-md-6">  <!-- //ขนาดตาราง -->
			<div class="panel panel-primary">
				...
				<form:form action="tool" method="POST"  id="tool" modelAttribute="tool"  commandName="tool" >
					<form:input path="quantity" type="hidden" value="0"/>
					<form:input path="quantityBorrow" type="hidden" value="0"/>
					<form:input path="quantityBroken" type="hidden" value="0"/>
		...
		function(isConfirm){   
			if (isConfirm) {
				...
				if($("#name").val() == ""){
					...
					$.post('chkTool',{name:$("#name").val()},function(msg){
						if(msg=="yes"){
				...
				$.post('addtool',$("#tool").serialize(),
				      function (result) {
						...
						else{swal("ผิดพลาด", "ชื่ออุปกรณ์ซ้ำ!", "error");}
					}) //end post
					
				}//end else in if confirm	
