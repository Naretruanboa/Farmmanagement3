					...
					<div class="input-group form-search">
						<select class="form-control  search-query" style="width: 150px; border-radius: 10px 0px 0px 10px; margin-right: -1px;"
								id="selectValue" onchange="changInput()">
									<option value="1">หมายเลขแม่พันธุ์</option>
						</select> 
				...
				<br>
					...
					<div class="panel-heading">
						<h4 class="panel-title">
							<span class="fa fa-list-alt fa-lg"></span> ข้อมูลการคลอด
						</h4>
					</div>
					<%
						int i = 0;
						...
						<table class="table table-striped table-hover  table-bordered"
							...
							<thead>
								<tr>
									...
									<th style="width: 190px;">หมายเลขแรกเกิด</th>
...
$("#listM").load('search_calve',{ id : 0 ,value : 0},function(){ //load list_view_medicine
...
function search(){ //click button search
	$("#listM").load('search_calve',{ id : $("#selectValue").val(), value : $("#search").val()},function(){
		$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
			for(i=0 ;i<=10;i++){
				if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
					$("[id=pageNum]").eq(i).addClass("active");
...
$(document).on('click','#pageNum',function(e){ // page number
	 ...
	 $("#listM").load('search_calve',{ id : $("#selectValue").val(), value : $("#search").val(),page:$("[id=pageNum]").eq(id).val()},function(){
			$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
				for(i=0 ;i<=10;i++){
					if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
						$("[id=pageNum]").eq(i).addClass("active");
