			...
			<div class="row form-group">
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						<form action="breed_edit" method="POST" id="breeding">
							<div class="row form-group">
								<div class="col-md-6">
									<fieldset><br>
									...
									<div class="row form-group">
											<label class="col-md-offset-s col-md-3 control-lable"><font color="red">* </font> หมายเลขแม่พันธุ์</label>
												<div class="col-md-6">
							 						...
							 						<form:select class="form-control" id="Cattle.maId" path="Cattle.maId">
														<form:option value="0">เลือกรายการ</form:option>
															<c:forEach items="${memcat}" var="mon">
															<c:if test="${Cattle.maId == mon.id }">
																	<form:option value="${Cattle.maId}" selected="selected">${mon.value}</form:option></c:if>
															
															<c:if test="${Cattle.maId == mon.id }">
														<form:option value="${Cattle.maId }">${mon.value}</form:option>
														</c:if>
									...
									<select class="form-control" id="memberid" name="memberid">
										<%-- <form:option value="0">ผู้บันทึก</form:option> --%>
												<c:forEach items="${ulist}" var="user">
														...
														<option value="${breeding.member.id }">${breeding.member.first }&nbsp;${breeding.member.last  }</option>
										...
										<div class="row form-group">
											...
											<div>
												<textarea rows="3" cols="45" class="form-control" name="note" id="note" value="${breeding.note}" type="text" ></textarea>
								...
								<div class="row form-group">
									<fieldset><br>
										...
										<div class="row form-group">
											...
											<div class="col-md-6">
												<input value="${breeding.date }"  name="date" id="date" type="date" class="form-control" />
											</div>
										</div>
										...
										<div class="row form-group">
											...
											<div class="col-md-6">
												<input value="${breeding.time }" name="time" id="time" type="time" class="form-control" />
											</div>
										</div>										
									</fieldset>
								</div>
...
<script type="text/javascript">
	...
	function cancel() {
		swal({	title: "Cancelled!",   
			...
			function(){   
				window.location.href='breed_edit'; 
		});
