						...
						<table class="table table-striped table-hover  table-bordered"
							...
							<tbody style="text-align: center;" id="list">
								...
								<c:forEach items="${weight }" var="weight">
									...
									<tr>
										<td><%=i++ %></td>
										<td>${weight.cattle.numDate }</td>
										<td>${weight.date }</td>
										<td>${weight.weightyear }</td>					
										<td>${weight.member.first }&nbsp;${weight.member.last }</td>
										<td>
										<form:form action="weight_detail?id=${weight.id }" method="POST" commandName="show">
											<input id="id" name="id" type="hidden"  value="${weight.id}">
										...
										<td>	
										<form:form action="weight_edit?id=${weight.id }" method="POST" >
										...
										<button class="btn btn-danger"	value="${weight.id}" onclick="chkdel(this.value)">
...
$("#list").load('search_weight2',{ id : 0 ,value : 0},function(){ //load list_view_medicine
...
function search(){ //click button search
	$("#list").load('search_weight2',{ id : $("#selectValue").val(), value : $("#search").val()},function(){
		$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
			for(i=0 ;i<=10;i++){
				if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
					$("[id=pageNum]").eq(i).addClass("active");
...
$(document).on('click','#pageNum',function(e){ // page number
	 ...
	 $("#list").load('search_weight1',{ id : $("#selectValue").val(), value : $("#search").val(),page:$("[id=pageNum]").eq(id).val()},function(){
			$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
				for(i=0 ;i<=10;i++){
					if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
						$("[id=pageNum]").eq(i).addClass("active");
