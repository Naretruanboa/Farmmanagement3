		...
		<div class=" row form-group">
				<div class="form-inline">
					...
					<div class="col-md-9" align="right">
						...
						</select> <input id="search" oninput="search()" class="form-control search-query "
...
$("#list").load('search_memberH',{ id : 0 ,value : 0},function(){
...
function search(){
	$("#list").load('search_memberH',{ id : $("#selectValue").val(), value : $("#search").val()},function(){
		$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
			for(i=0 ;i<=8;i++){
				if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
					$("[id=pageNum]").eq(i).addClass("active");
...
$(document).on('click','#pageNum',function(e){
	 ...
	 $("#list").load('search_memberH',{ id : $("#selectValue").val(), value : $("#search").val(),page:$("[id=pageNum]").eq(id).val()},function(){
			$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
				for(i=0 ;i<=8;i++){
					if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
						$("[id=pageNum]").eq(i).addClass("active");
