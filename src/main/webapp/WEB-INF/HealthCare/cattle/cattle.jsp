												...
												<form:select class="form-control" id="typeCattle.id" path="typeCattle.id">
													<form:option value="">ประเภทของโค</form:option>
															<c:forEach items="${type}" var="typecat">
																...
																<c:if test="${typeCattle.id != typecat.id }">
																<form:option value="${typecat.id }">${typecat.value} // ${typecat.id}</form:option>
																<c:set var = "num" scope = "session"  value = "${typecat.id }"/>
																</c:if>
										 ...
										 <div>
											<form:select class="form-control" id="color.id" path="color.id">
												<form:option value="0">คอก</form:option>
														<c:forEach items="${stal}" var="stalG">
															<c:if test="${stalG.id == stalG.id }">
																<form:option value="${stalG.id}" selected="selected">${stalG.value}</form:option></c:if>
																		
															<c:if test="${stalG.id != stalG.id }">
															<form:option value="${stalG.id }">${stalG.value}</form:option>
															</c:if>
