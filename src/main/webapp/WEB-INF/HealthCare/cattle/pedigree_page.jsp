...
<title>ข้อมูลพันธุ์ประวัติโค</title>
</head>
<body>
...
<div class="container">
	<div class="col-md-3">
			<div class="jumbotron" style="background-color: rgb(218, 68, 83)" align="center">
				<p style="font-size: 28px; font-weight: 700; color: white;">ข้อมูลพันธุ์ประวัติโค</p>						
			</div>		
		</div>
	<div class="row">
		<hr>
				...
				<label for="select">ค้นหาโดย</label>
					<div class="input-group form-search">
						<select class="form-control  search-query" style="width: 150px; border-radius: 10px 0px 0px 10px; margin-right: -1px;"
								...
								<option value="3">เบอร์แรกเกิด</option>
						</select> 
							<span class="input-group-btn"> <input type="text" class="form-control search-query" id="search" oninput="search()" >
				...
				<br>
					...
					<div class="panel-heading">
						<h4 class="panel-title">
							<span class="fa fa-list-alt fa-lg"></span> ข้อมูลพันธุ์ประวัติโค
						</h4>
					</div>
					<%
						int i = 0;
						...
						<table class="table table-striped table-hover  table-bordered"
							...
							<thead>
								<tr>
									...
									<th style="width: 190px;">ชื่อโค</th>
									<th style="width: 190px;">วัน/เดือน/ปี เกิด</th>
									<th style="width: 190px;">สายพันธุ์</th>
									<th style="width: 190px;">สีโค</th>
									<th style="width: 190px;">ดูรายละเอียด</th>
							...
							<tbody style="text-align: center;" id="listP">
...
$("#listP").load('search_pedigree',{ id : 0 ,value : 0},function(){ //load list_view_medicine
...
function search(){ //click button search
	$("#listM").load('search_cattle',{ id : $("#selectValue").val(), value : $("#search").val()},function(){
		$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
			for(i=0 ;i<=10;i++){
				if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
					$("[id=pageNum]").eq(i).addClass("active");
...
$(document).on('click','#pageNum',function(e){ // page number
	 ...
	 $("#listM").load('search_cattle',{ id : $("#selectValue").val(), value : $("#search").val(),page:$("[id=pageNum]").eq(id).val()},function(){
			$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
				for(i=0 ;i<=10;i++){
					if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
						$("[id=pageNum]").eq(i).addClass("active");
