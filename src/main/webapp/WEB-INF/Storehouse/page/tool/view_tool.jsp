				...
				<div class="form-inline">
					<div class="col-md-3">
						<div class="jumbotron" style="background-color: rgb(218, 68, 83)"
							align="center">
							<p style="font-size: 28px; font-weight: 700; color: white;">ข้อมูลวัสดุ/อุปกรณ์</p>
						</div>
					</div>
...
<div class="panel panel-primary">
		...
		<div>
		<a href="add_tool"><button type="button" class="btn btn-success">ลงทะเบียนวัสดุ/อุปกรณ์</button></a>
		</div><br>
...
$("#list").load('search_tool',{ id : 0 ,value : 0},function(){
...
function search(){
	$("#list").load('search_tool',{ id : $("#sel").val(), value : $("#search").val()},function(){
		$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
			for(i=0 ;i<=8;i++){
				if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
					$("[id=pageNum]").eq(i).addClass("active");
...
$(document).on('click','#pageNum',function(e){
	 ...
	 $("#list").load('search_tool',{ id : $("#sel").val(), value : $("#search").val(),page:$("[id=pageNum]").eq(id).val()},function(){
			$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
				for(i=0 ;i<=8;i++){
					if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
						$("[id=pageNum]").eq(i).addClass("active");
