				...
				<div class="form-inline">
					<div class="col-md-3">
						<div class="jumbotron" style="background-color: rgb(218, 68, 83)"
							align="center">
							<p style="font-size: 28px; font-weight: 700; color: white;">การเบิกวัสดุที่ใช้ในฟาร์ม</p>
						</div>
					</div>
					<div class="col-md-9" align="right">
						...
						<span class="input-group-btn search-query"> 
						 <select class="form-control  search-query btn-info" id="selectValue1" style="width: 150px;color: white;">
								<option value="1">ชื่อวัสดุ</option>	
						</select>
...
<div class="panel panel-primary">
		<div class="panel-heading">
	        <h4 class="panel-title" style="font-size: 25px;">วัสดุคงคลัง</h4>
	   	</div>
		...
		<div class=" row form-group">
				<div class="form-inline">
					<div class="col-md-3">
						<div class="jumbotron" style="background-color: rgb(218, 68, 83)"
							align="center">
							<p style="font-size: 28px; font-weight: 700; color: white;">ประวัติการเบิกวัสดุที่ใช้ในฟาร์ม</p>
						</div>
					</div>
					<div class="col-md-9" align="right">
						<div class="input-group form-search">
						...
						<span class="input-group-btn search-query"> <select class="form-control  search-query btn-info" id="selectValue" style="width: 150px;color: white;">								
								...
								<option value="2">วันที่เบิก</option>
...
//search for release ####################################################
$("#listRelease").load('search_release_t',{ id : 1 ,value : null},function(){ 
...
function search1(){
	$("#listRelease").load('search_release_t',{ id : $("#selectValue1").val(), value : $("#search1").val()},function(){
		$("#p").load('page1',{ pagePresent1 : $("#Page1").val(), pageLast1 : $("#countPage1").val()},function(m){
			for(i=0 ;i<=10;i++){
				if($("[id=pageNum1]").eq(i).val()==$("#Page1").val()){
					$("[id=pageNum1]").eq(i).addClass("active");
...
$(document).on('click','#pageNum1',function(e){ 
	 ...
	 $("#listRelease").load('search_release_t',{ id : $("#selectValue1").val(), value : $("#search1").val(),page:$("[id=pageNum1]").eq(id).val()},function(){
			$("#p").load('page1',{ pagePresent1 : $("#Page1").val(), pageLast1 : $("#countPage1").val()},function(m){
				for(i=0 ;i<=10;i++){
					if($("[id=pageNum1]").eq(i).val()==$("#Page1").val()){
						$("[id=pageNum1]").eq(i).addClass("active");
...
//search release
$("#list").load('search_release_tool',{ id : 0 ,value : 0},function(){ 
...
function search(){ 
	$("#list").load('search_release_tool',{ id : $("#selectValue").val(), value : $("#search").val()},function(){
		$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
			for(i=0 ;i<=10;i++){
				if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
					$("[id=pageNum]").eq(i).addClass("active");
...
$(document).on('click','#pageNum',function(e){ 
	 ...
	 $("#list").load('search_release_tool',{ id : $("#selectValue").val(), value : $("#search").val(),page:$("[id=pageNum]").eq(id).val()},function(){
			$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
				for(i=0 ;i<=10;i++){
					if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
						$("[id=pageNum]").eq(i).addClass("active");
