...
<div class="panel panel-primary">
		...
		<div>
		<a href="add_medicine"><button type="button" class="btn btn-success">ลงทะเบียนยาใหม่</button></a>
		</div><br>
...
<script>
$("#listM").load('search_medicine',{ id : 0 ,value : 0},function(){ //load list_view_medicine
...
$(document).on('click','#pageNum',function(e){ // page number
	 ...
	 $("#listM").load('search_medicine',{ id : $("#selectValue").val(), value : $("#search").val(),page:$("[id=pageNum]").eq(id).val()},function(){
			$("#page").load('page',{ pagePresent : $("#Page").val(), pageLast : $("#countPage").val()},function(m){
				for(i=0 ;i<=10;i++){
					if($("[id=pageNum]").eq(i).val()==$("#Page").val()){
						$("[id=pageNum]").eq(i).addClass("active");
